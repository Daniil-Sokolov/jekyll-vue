...
module Jekyll
  ...
  module Filters
    ...
    def category_links(categories)
      ...
      # allow next line for prod?????
      base_url = @context.registers[:site].config['baseurl']
      base_url = base_url ? base_url : '/'
      #base_url = "" if (base_url == nil) else base_url

      base_dir = File.join(base_url, cat_dir)
      ...
      categories = categories.sort!.map do |category|
        ...
        # #"<pre>category_dir = #{category_dir}<br />base_url = #{base_url}<h2>Config from Plugin</h2>#{conf}</pre>";
